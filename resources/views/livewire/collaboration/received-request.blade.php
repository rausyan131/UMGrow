    ...
    @if ($requests->isEmpty())
        @if (empty($searchInput) && empty($statusInput))
            <div class="glass p-8 rounded-xl text-center text-gray-400 italic shadow-md">
                <i class="fas fa-envelope-open-text text-4xl mb-4 text-primary"></i>
                <p class="text-lg font-semibold text-gray-500 dark:text-gray-300">Belum ada permintaan masuk</p>
                <p>UMKM lain belum mengajukan kolaborasi ke kamu, </p>
            </div>
        @else
            <div class="glass p-8 rounded-xl text-center text-gray-400 italic shadow-md">
                ...
                <p>
                    Tidak ada permintaan kolaborasi yang cocok dengan pencarian <strong>"{{ $searchInput }}"</strong>
                    @if ($statusInput)
    ...
    @else
        @foreach ($requests as $request)
            <div
                ...
                <a href="{{ route('collaboration.received-request.detail', $request->id) }}" class="w-full">
                        ...
                        <div class="space-y-2 flex gap-6 items-center">
                            ...
                            <div>
                                ...
                                <h2 class="text-lg font-semibold text-text dark:text-white flex items-center gap-2">
                                    Dari: {{ $request->partnerUmkm->umkm_name ?? 'UMKM Tidak Ditemukan' }}
                                </h2>
